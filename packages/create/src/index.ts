...
export async function stageYarnBin(options: StageYarnOptions): Promise<string | undefined> {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const stream = Readable.fromWeb(resp.body as any).pipe(
        tar.extract(
          {
            cwd: tempDir,
            newer: true,
...
class Scaffold {
  ...
  async scaffold() {
    ...
    await new Promise<void>((resolve, reject) => {
      const stream = Readable.fromWeb(resp2.body as any).pipe(
        tar.extract({
          cwd: rootDir,
          newer: true,
          strip: 1,
  ...
  async start() {
    console.log();
    console.log(
      `  ${kleur.bold(`create ${this.options.name}`)}  ${kleur.blue(`v${this.options.version}`)}`,
    );
    console.log();
