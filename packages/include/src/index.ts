...
export class Include extends EntryTree {
  ...
  async* [Service.init]() {
    try {
      ...
      const cause
        = error instanceof ConfigFileError && error.stage === 'read' ? error.cause : undefined
  ...
  private async _read() {
    ...
    try {
      data
        = this.type === 'application/yaml' ? yaml.load(content, { schema }) : JSON.parse(content)
    } catch (error) {
    ...
    if (this.cache) {
      ...
      const conflicts = reconcile(
        this.journal,
        flatten(this.cache.data),
        flatten(data),
        (id, key) => index.fileOwned(id, key),
      )
  ...
  private async _write() {
    ...
    if (current === undefined) {
      this.ctx.logger.warn(
        'config file %C is missing, %d pending change(s) kept in memory',
        this.filename,
        this.journal.size,
      )
      return
    ...
    if (routeJournal(batch, data, patches, this.warn)) {
      ...
      if (this.ctx.fiber.entry) {
        ...
        this.ctx.logger.warn(
          'patches of %C changed at runtime but cannot be persisted',
          this.filename,
        )
      }
    }
    ...
    try {
      ...
      restore()
      this.ctx.logger.warn(
        'config file %C is read-only, %d pending change(s) kept in memory',
        this.filename,
        this.journal.size,
      )
      return
    ...
    try {
      ...
      this.ctx.logger.warn(
        'failed to write config file %C, %d pending change(s) kept in memory',
        this.filename,
        this.journal.size,
      )
      throw error
  ...
  private async _writeText(text: string, expected?: string) {
    const tmp = join(
      ...
      `.${basename(this.filename)}.${process.pid}.${this._seq++}.tmp`,
    )
    try {
