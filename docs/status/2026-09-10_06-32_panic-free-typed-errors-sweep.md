...
3. **Prefer explicit per-site edits over line-numbered sed** when the edit encodes a _policy_ (which sites keep panicking).
...
2. **End-state for the remaining 27 Zig dispatch panics (and the sharedKey/rootKey leakage):** is "dispatch-boundary abort" an acceptable _permanent_ end state, or do you want the full conversion — including changing the shared dispatch callback contract across all three ports — to reach literal zero panics outside `Must*` helpers?
