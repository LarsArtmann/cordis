...
1. **Chat-time verification gate:** any sentence about external tool behavior, versions, or statistics gets a primary-source check or an explicit "plausible, untested" label — *before* sending, not after a challenge.
...
**cordis × Go 1.27 (P1):**
1. Migrate typed free functions to generic methods on `*Context`: `ctx.Get[T]()`, `ctx.TryGet[T]()`, `ctx.MustGet[T]()`, `ctx.Provide[T]()`, `ctx.On[E]()`, `ctx.Once[E]()`, `ctx.Emit[E]()`; keep free functions as deprecated aliases during transition.
2. Verify golden traces unchanged after the API migration (Go/Rust/Zig must stay byte-identical).
3. Document in AGENTS.md which 1.27 feature(s) actually pin the `go 1.27` directive.
4. Add a `goroutineleak` pprof assertion to drain/dispose/rollback tests.
5. Use `testing/synctest.Sleep` in `go/timer` tests where `time.Sleep` + `Wait` combos exist.
...
9. Composition root wrapper returning a cleanup function (DO-2).
10. `do.ProvideValue` for config/logger/DB (eager foundation).
11. Lifecycle guards: `var _ do.ShutdownerWithError = (*MyService)(nil)`.
12. Wire `samber-do-auditlog` via `Add*Hook`s.
13. `ShutdownOnSignals` for daemon mode.
14. Write the ~50-line broadcaster for `internal/webui/hub.go` instead of adopting `ro`.
15. `goroutineleak` CI profile after `Shutdown()` in tests.
16. Audit DO-1…DO-6 compliance once wired.

**Docs / memory (P2):**
17. HARVEST this section into TODO_LIST.md / ROADMAP.md (docs-health).
18. Record the samber/* decision + verified stats in go-taskqueue's docs.
19. Update `samber-do-best-practices` skill verification block (v2.1.0, 342 importers, rollback repro result, date).
20. ROADMAP entry: native-max API phase 3 (generic methods), with divergence note (Rust already has generic methods; Zig comptime — no divergence needed).
21. Deprecation timeline decision for the free functions (aliases vs removal pre-v1).
...
25. Evaluate stdlib `uuid` for any cordis uid needs (probably keep uint64 keys).
26. `encoding/json/v2` adoption pass for the Go port (likely nothing to do — verify).
...
29. Confirm the auto-commit daemon picked up nothing unexpected this session (tree was clean at 07:46).
...
2. **Timing:** adopt the generic-method API migration in cordis now (pre-v1 window, rebase just completed) or park it in ROADMAP behind higher-parity work?
3. **Persistence:** write the verified facts (do v2.1.0 / 342 importers / rollback semantics / 1.27 feature list) into the `samber-do-best-practices` skill and ROADMAP now, or keep this session chat-only?

---

*Generated 2026-09-08 07:46 CEST. Not committed (no commit was requested); the auto-git daemon will pick it up.*
