...
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------ | ----------------------------------------------------------- | ---------------------------------------------------------- |
| ~~M01~~ | ~~Push `main`, watch the first `ports.yml` run, record the green URL in TODO_LIST~~ done at `3dcc565`                                                                                                                           | ~~P1~~   | ~~Critical~~ | ~~30m~~                                                     | ~~All session work becomes remotely proven~~               |
| ~~M02~~ | ~~CI hardening: `golangci-lint` job, `go test -count=3`, golden double-run canary, actionlint re-verify~~ done at `7b4abe9`, `eab03a8`                                                                                          | ~~P1~~   | ~~High~~     | ~~30m~~                                                     | ~~No silent lint regressions; flake canary~~               |
| ~~M03~~ | ~~Rust `isolate_shared` collision-free labels (tuple-keyed table like Go) + collision tests~~ done — Rust shared-key label table + collision tests (rust/src/core.rs)                                                           | ~~P4~~   | ~~High~~     | ~~30m~~                                                     | ~~Kills the known bug class in Rust~~                      |
| ~~M04~~ | ~~Zig `isolateShared` collision-free labels + collision tests~~ done — Zig PairKey label table + collision tests                                                                                                                | ~~P4~~   | ~~High~~     | ~~30m~~                                                     | ~~Kills the known bug class in Zig~~                       |
| ~~M05~~ | ~~Zig `Context.effect` nested-scope API + effect-label introspection (`EffectMeta` trees)~~ done — Zig Context.effect + effects() introspection                                                                                 | ~~P4~~   | ~~High~~     | ~~90m~~                                                     | ~~Zig reaches Go/Rust effect parity~~                      |
| ~~M06~~ | ~~Zig Disposer handles for `on`/`provide` + registry view (size/has/delete) + typed `once`~~ done — Zig Disposers, registry view, onceTyped                                                                                     | ~~P4~~   | ~~High~~     | ~~90m~~                                                     | ~~Zig becomes usable as a port~~                           |
| ~~M07~~ | ~~Golden scenario #2: typed/string events, filters, global listeners — three runners, byte-identical~~ done at `7b4abe9`                                                                                                        | ~~P4~~   | ~~High~~     | ~~60m~~                                                     | ~~Event parity machine-enforced~~                          |
| ~~M08~~ | ~~Golden scenario #3: nested plugins + registry delete cascade — three runners~~ done at `7b4abe9`                                                                                                                              | ~~P4~~   | ~~Medium~~   | ~~60m~~                                                     | ~~Cascade parity machine-enforced~~                        |
| ~~M09~~ | ~~Go coverage → ≥90% (Waterfall panic guards, Parallel recover, slog edges, failed-load stdctx)~~ done at `7b4abe9`                                                                                                             | ~~P4~~   | ~~Medium~~   | ~~90m~~                                                     | ~~Trust in the flagship's error paths~~                    |
| ~~M10~~ | ~~DSL parser unit tests for all three golden runners (malformed tokens, param edge cases)~~ done at `7b4abe9`                                                                                                                   | ~~P4~~   | ~~Medium~~   | ~~30m~~                                                     | ~~Golden runners stop being triplicated hand-rolled risk~~ |
| ~~M11~~ | ~~Go port `packages/timer` (interval/timeout effects on `Fiber.StdContext`)~~ done at `8e30a83`, `7b4abe9`                                                                                                                      | ~~P20~~  | ~~Medium~~   | ~~30m~~                                                     | ~~First ecosystem package~~                                |
| ~~M12~~ | ~~Go port `packages/group` (join/leave with registry semantics)~~ done at `8e30a83`, `7b4abe9`                                                                                                                                  | ~~P20~~  | ~~Medium~~   | ~~30m~~                                                     | ~~Ecosystem package~~                                      |
| ~~M13~~ | ~~Go `internal/get~~ done at`8e30a83`,`7b4abe9`                                                                                                                                                                                 | ~~set~~  | ~~listener~~ | ~~dispatch` interception events (loader/hmr prerequisite)~~ | ~~P20~~                                                    |
| ~~M14~~ | ~~Go loader part 1: config schema, file discovery, plugin resolution, validation, start pipeline~~ done at `b87a305`                                                                                                            | ~~P20~~  | ~~High~~     | ~~100m~~                                                    | ~~Config-driven plugin management~~                        |
| ~~M15~~ | ~~Go loader part 2: watch/reload diffing, interception reactions, rollback tests~~ done at `f07fa20`                                                                                                                            | ~~P20~~  | ~~High~~     | ~~100m~~                                                    | ~~Live reload on config change~~                           |
| ~~M16~~ | ~~Go port `packages/hmr` (module identity, accept/decline, dispose+relink, TS fixtures)~~ done at `7f2d8b1`                                                                                                                     | ~~P20~~  | ~~High~~     | ~~100m~~                                                    | ~~The killer upstream feature~~                            |
| ~~M17~~ | ~~Rust thread-safe variant part 1: `Arc`/`Mutex` core, lock-free user callbacks preserved~~ done at `8e30a83`, `d93f236`                                                                                                        | ~~P20~~  | ~~High~~     | ~~100m~~                                                    | ~~Unlocks real (threaded) Rust usage~~                     |
| ~~M18~~ | ~~Rust thread-safe variant part 2: feature-flag wiring, stress tests, clippy~~ done at `d93f236`                                                                                                                                | ~~P20~~  | ~~High~~     | ~~60m~~                                                     | ~~Ship the variant safely~~                                |
| ~~M19~~ | ~~Zig `serial` + `waterfall` dispatch modes + tests~~ done — zig serial + waterfall in cordis.zig                                                                                                                               | ~~P20~~  | ~~Medium~~   | ~~90m~~                                                     | ~~Event story completion~~                                 |
| ~~M20~~ | ~~Zig `parallel` dispatch (error joining) + `batch` transactions + tests~~ done — zig parallel + batch in cordis.zig                                                                                                            | ~~P20~~  | ~~Medium~~   | ~~90m~~                                                     | ~~Event + transaction story completion~~                   |
| ~~M21~~ | ~~Rust `intercept`/`Intercepted` + config validation hook + typed `update`~~ done — Rust intercept/validate/update_config                                                                                                       | ~~P100~~ | ~~Medium~~   | ~~90m~~                                                     | ~~Go-parity scope configuration~~                          |
| ~~M22~~ | ~~Rust registry snapshot restore + `internal/status` event emission + typed doctests~~ done at `9b27371`                                                                                                                        | ~~P100~~ | ~~Medium~~   | ~~90m~~                                                     | ~~Tooling parity + docs~~                                  |
| ~~M23~~ | ~~Rust true parallel dispatch with scoped threads~~ done — Rust parallel via scoped threads under thread-safe                                                                                                                   | ~~P100~~ | ~~Medium~~   | ~~60m~~                                                     | ~~Performance parity with Go `Parallel`~~                  |
| ~~M24~~ | ~~Go accessor/mixin service system (Property.Accessor upstream)~~ done at `2fd2f2f`                                                                                                                                             | ~~P100~~ | ~~Medium~~   | ~~100m~~                                                    | ~~Advanced service patterns~~                              |
| ~~M25~~ | ~~Go callable services + tracker-based effect attribution~~ done at `053a72a`                                                                                                                                                   | ~~P100~~ | ~~Medium~~   | ~~100m~~                                                    | ~~Effects created via services attributed to callers~~     |
| ~~M26~~ | ~~Go API polish + docs batch: `Fiber.Err()`, `Await(ctx)`, `errors.AsType`, typed-inject sugar, PORTS API table, README badges, AGENTS Zig gotchas, Batch semantics + upstream notes~~ done at `8efd0f6`                        | ~~P100~~ | ~~Medium~~   | ~~90m~~                                                     | ~~Ergonomics + documentation depth~~                       |
| ~~M27~~ | ~~Quality batch: flake-check CI job, benchmark skeletons (Go first), LIFO rollback property test, releases/tags, `zig build -femit-docs`, stale-LSP/AsType sweep, run cadence note, parity-matrix generator~~ done at `df35fc4` | ~~P100~~ | ~~Low~~      | ~~100m~~                                                    | ~~Long-term health~~                                       |
