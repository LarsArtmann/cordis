...
Everything from the phase-2 session is *claimed* done but **remotely
...
The ports are *almost* interchangeable. Three real gaps remain:
...
events they need) is what makes someone *choose* this framework. In
...
| M13 | Go `internal/get|set|listener|dispatch` interception events (loader/hmr prerequisite) | P20 | Critical | 100m | Unblocks loader + hmr |
...
| F13.1 | Define `internal/get|set|listener|dispatch` event names + emit points in store ops | 12m | — |
...
*Point-in-time plan. Re-verify claims against the repo before executing.
Generated 2026-09-04 15:42 CEST.*
